...
public class UpdateAppleGuestTokenOrderPlaced extends CreateAppleGuestToken{

    static String auth_token= createAGToken();
    public static void updateAGTokenOrderPlaced(){
        ...
        requestParams.put( "token_status_code" , "session_created");
        ...
        System.out.println("------>> Update Apple Guest Token Order Placed Response Body is: ");
        String responseBody = response.getBody().prettyPrint();
        int statusCode= response.getStatusCode();
        System.out.println("Status Code is: "+ statusCode);
        Assert.assertEquals(200, statusCode);
        String tokenStatusCode = response.jsonPath().get("data.token_status_code");
        Assert.assertEquals("order_placed",tokenStatusCode);
